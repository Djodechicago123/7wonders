...
// Imports statiques — évite les problèmes de glob en production
...
export default function WonderBoard({ player, compact = false }) {
  ...
  const boardUrl = getBoardUrl(wonder);
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="text-right">
              <p className="font-display font-black text-2xl text-gold-400"
                style={{ textShadow: '0 0 10px rgba(212,172,13,0.6)' }}>
...
function renderStageEffect(effect, big = false) {
  ...
  const parts = [];
  if (effect.vp) parts.push(<span key="vp" className={`${sz} text-yellow-400 flex items-center gap-0.5`}>⭐<span>{effect.vp}</span></span>);
  if (effect.coins) parts.push(<span key="coins" className={`${sz} text-yellow-300 flex items-center gap-0.5`}>🪙<span>{effect.coins}</span></span>);
  if (effect.shields) parts.push(<span key="shields" className={`${sz} text-red-400 flex items-center gap-0.5`}>🛡<span>{effect.shields}</span></span>);
